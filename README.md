...
weekly-relay プロジェクトの Firestore は廃止済み。すべて SmartSync Firestore（検証: `smart-sync-stg` / 本番: `smart-sync`）に保存する。
...
| `wasabi_sync_logs` / `wasabi_job_status` / `wasabi_audit_logs` | 実行ログ・鮮度管理・監査 |
...
| Backlog 転記（手動） | `転記` / `転記 7/1-7/4` | プレビュー表示 → 確認ボタンで実行 |
