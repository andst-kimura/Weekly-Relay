...
| ナレッジ蓄積 | 議事録・チケット・Slack が散在 | Firestore へ自動蓄積・ChromaDB で意味検索 |
| 未対応チケット管理 | 手動でBacklogを確認 | N営業日更新なしで Slack DM 通知 |
| KB への質問 | 過去資料を手動検索 | Slack Bot に質問 → Gemini が KB を検索して回答（RAG） |
| 進捗の事前インプット | 週次実行まで反映されない | Slack Bot にメモを送ると次の週次レポートに自動反映 |
...
| 操作 | チャンネル | DM |
|---|---|---|
| KB 質問（RAG） | `@Weekly Relay ACE刷新の進捗は？` | `ACE刷新の進捗は？` |
| 進捗メモ登録 | `@Weekly Relay メモ SALES_TEAM-23: 内容` | `メモ SALES_TEAM-23: 内容` |
| Bot 返信を削除 | 返信スレッドで `@Weekly Relay delete` | `delete` |
| ヘルプ表示 | `@Weekly Relay help` | `help` |
...
@Weekly Relay メモ SALES_TEAM-23: ポスタスの検証環境エラーは解消済み。6/27 本番反映予定

# 課題キーを省略する場合（関連する全親課題の転記に追加）
@Weekly Relay メモ ポスタスの検証環境エラーが解消した
```
...
1. Slack の左サイドバーで **Weekly Relay**（Bot名）を検索
...
| 操作 | チャンネル | DM |
|---|---|---|
| KB 質問 | `@Weekly Relay ACE刷新の進捗は？` | `ACE刷新の進捗は？` |
| 進捗メモ | `@Weekly Relay メモ SALES_TEAM-23: 内容` | `メモ SALES_TEAM-23: 内容` |
| Bot 返信削除 | 返信スレッドで `@Weekly Relay delete` | `delete` |
| ヘルプ | `@Weekly Relay help` | `help` |
