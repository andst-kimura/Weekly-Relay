...
| AI エンジン | Gemini API（`gemini-2.5-flash` / 埋め込み: `text-embedding-004`） |
...
| ドキュメントID形式 | 内容 | TTL |
|---|---|---|
| `ticket_{ISSUE-KEY}` | チケット要約・コメント履歴 | なし（永続） |
| `slack_{YYYYWW}_{channel_name}` | Slack チャンネル週次まとめ | なし（永続） |
| `meeting_{YYYYMMDD}_{doc_id末尾12文字}` | 議事録全文・AI要約 | なし（永続） |

> `sync_logs`（実行ログ）のみ TTL 90日を設定。
...
検索結果はドキュメントID・類似スコア・テキスト抜粋をログ出力します。
...
│   ├── gemini_client.py            # Gemini API クライアント（要約・判別・転記・埋め込み）
