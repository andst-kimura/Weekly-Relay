...
| ナレッジ蓄積 | 議事録・チケット・Slack が散在したまま | Firestore へ自動蓄積・ChromaDB でベクトルインデックス化 |
...
| 機能 | 手動（今まで） | このシステム |
|---|---|---|
| 過去情報の検索 | Backlog・Slack・ドキュメントを個別に手動検索 | Slack Bot に質問 → KB を意味検索して Gemini が回答（RAG） |
...
| 請求書メールの確認 | 受信トレイを目視確認 | Bot コマンドで今月の請求書メールを一覧化（既知送信元 + Gemini 判定） |
| 未返信メールの把握 | 定期的に受信トレイを目視確認 | Bot コマンドで N 日以上放置の要返信メールを Gemini が判定して通知 |
...
| Google 連携 | Google Calendar / Drive / Docs / Gmail API（OAuth2） |
| データ永続化 | Firestore REST API（gRPC 不使用・`requests` ベース） |
| 意味検索 | ChromaDB（ローカル永続化）+ Gemini Embeddings |
...
sequenceDiagram
    ...
    SCH->>FS: Step 9: ナレッジベース生成（tickets / slack / meetings）+ ChromaDB 同期
```
...
| 操作 | チャンネル | DM |
|---|---|---|
| KB 質問（RAG） | `@Wasabi Bot ACE刷新の進捗は？` | `ACE刷新の進捗は？` |
| 進捗メモ登録 | `@Wasabi Bot メモ SALES_TEAM-23: 内容` | `メモ SALES_TEAM-23: 内容` |
| 共有事項の起票 | `@Wasabi Bot 共有事項 タイトル: 内容` | `共有事項 タイトル: 内容` |
| 請求書メール一覧 | `@Wasabi Bot 請求書` | `請求書` |
| 未返信メールチェック | `@Wasabi Bot 未返信チェック` | `未返信チェック` |
| Bot 返信を削除 | 返信スレッドで `@Wasabi Bot delete` | `delete` |
| ヘルプ表示 | `@Wasabi Bot help` | `help` |

### 進捗メモの詳細

Slack Bot に登録したメモは Firestore `manual_memos` コレクションに保存され、次の週次レポート実行時（金曜 18:00）に情報源として使用されます。

```
# 課題キーを指定する場合（その親課題の転記に反映）
@Wasabi Bot メモ SALES_TEAM-23: ポスタスの検証環境エラーは解消済み。6/27 本番反映予定

# 課題キーを省略する場合（関連する全親課題の転記に追加）
...
今月受信した請求書メールを一覧表示します。判定は**既知送信元の一致**と**Gemini による本文判定**の合わせ技で行います。
...
1. 【ご請求書】6月分 クラウドサービス利用料
   差出人: billing@example.com
   受信日: 2026/06/10 09:32
...
```

既知の送信元は `config.yaml` の `gmail.invoice_senders` に追加します。ここに含まれる送信元は Gemini 判定をスキップして即採用されます。

### Gmail：未返信メールチェック

受信トレイにある N 日以上放置されたメールのうち、Gemini が「返信が必要」と判断したものを一覧表示します。
...
1. 【確認依頼】来週のスケジュールについて
   差出人: tanaka@example.com
   受信日: 2026/06/22 14:15
...
```

判定日数は `config.yaml` の `gmail.unreplied_days` で変更できます。

> **初回利用時**: `config/google_token.pickle` を削除し、`python main.py --bot` を再起動してください。ブラウザが開き Gmail スコープを含む再認証が行われます。
...
## ナレッジベース（Firestore + ChromaDB）

KB 生成で作成されるドキュメントは Firestore `context_snapshots` コレクションに永続保存され、同時に ChromaDB へベクトル化して蓄積されます。

### Firestore コレクション構造

| コレクション | ドキュメントID形式 | 内容 | TTL |
|---|---|---|---|
| `context_snapshots` | `ticket_{ISSUE-KEY}` | チケット要約・コメント履歴 | なし（永続） |
| `context_snapshots` | `slack_{YYYYWW}_{channel_name}` | Slack チャンネル週次まとめ | なし（永続） |
| `context_snapshots` | `meeting_{YYYYMMDD}_{doc_id末尾12文字}` | 議事録全文・AI要約 | なし（永続） |
| `manual_memos` | `memo_{YYYYMMDD}_{timestamp_ms}` | Slack Bot 経由の手動メモ | なし（永続） |
| `sync_logs` | （自動採番） | 実行ログ | TTL 90日 |
...
KB 保存と同時にベクトル化して `output/chroma/` にローカル保存します。

```bash
# 意味検索（--only kb 実行後から利用可）
python main.py --search "ACE刷新の進捗"

# Firestore の既存データを一括 ChromaDB に同期
...
│   ├── slack_bot.py                # Slack Bot（Socket Mode・KB 質問応答・進捗メモ・Gmail連携）
│   ├── google_calendar_client.py   # Google Calendar API クライアント
│   ├── google_docs_client.py       # Google Drive / Docs API クライアント（議事録取得）
│   ├── gmail_client.py             # Gmail API クライアント（請求書・未返信チェック）
│   ├── gemini_client.py            # Gemini API クライアント（要約・判別・転記・RAG・埋め込み）
│   ├── report_generator.py         # レポート生成（Gemini / ルールベース）・議事録並列要約
│   ├── knowledge_base.py           # ナレッジベース生成（Firestore + ChromaDB 保存）
│   ├── firestore_client.py         # Firestore REST API クライアント（gRPC 不使用）
│   ├── vector_store.py             # ChromaDB ベクトルストア（意味検索）
...
2. 以下の API を有効化：
   ...
   - Google Drive API
   - Google Docs API
   - Gmail API（Bot の請求書・未返信チェック機能を使う場合）
...
> **スコープを追加・変更した場合**: 既存の `config/google_token.pickle` を削除してから再実行してください。ブラウザが開き、まとめて再認証できます。
...
個人用 GCP プロジェクト（`weekly-relay`）に Named Database `weekly-relay` を作成します。
...
gmail:
  my_email: "yourname@example.com"       # 未返信判定で使用
  invoice_senders:                       # 請求書の既知送信元（Gemini 判定なしで採用）
    - "billing@example.com"
    - "freee.co.jp"
  unreplied_days: 3                      # N日以上放置を未返信チェック対象にする
...
| `kb` | ナレッジベース生成（Firestore `context_snapshots` への保存 + ChromaDB 同期） |
...
Firestore に既存データがある状態で ChromaDB を新規構築する場合に使用します。
...
| `ModuleNotFoundError: chromadb` | chromadb 未インストール | `pip install -r requirements.txt` を再実行 |
| `ModuleNotFoundError: slack_bolt` | slack-bolt 未インストール | `pip install -r requirements.txt` を再実行 |
| `--search` で結果が0件 | ChromaDB インデックス未構築 | `--only kb` または `--sync-vectors` を先に実行 |
...
| `請求書` / `未返信チェック` で Gmail エラー | Google トークンに `gmail.readonly` スコープがない | `config/google_token.pickle` を削除して `--bot` を再起動し再認証 |
| Gmail API が `accessNotConfigured` | Gmail API が GCP で未有効 | Google Cloud Console → API ライブラリ → Gmail API を有効化 |
| 請求書が0件で返ってくる | 件名・送信元が条件に合わない | `gmail.invoice_senders` に送信元ドメインを追加するか、件名に「請求書」「invoice」を含む確認 |
