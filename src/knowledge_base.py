...
class KnowledgeBase:
    def __init__(self, backlog_client: BacklogClient, slack_client: SlackClient,
                 ...
                 vector_store=None, team_members: list[dict] = None):
        self.backlog = backlog_client
        self.slack = slack_client
        self.gemini = gemini_client
        self.fs = firestore_client
        self.vs = vector_store        # VectorStore | None
        # team_members: [{"name": "田中", "backlog_user_id": 789, "slack_user_id": "U..."}]
    ...
    def generate(self, activities: list[dict], since: datetime, until: datetime,
        ...
        # Slack KB: チームメンバーが設定されている場合はチーム全員分収集
        team_slack_ids = (
            [self.slack.my_user_id] + [m["slack_user_id"] for m in self.team_members if m.get("slack_user_id")]
            if self.team_members else None
        )
        self._generate_slack_knowledge(since, until, team_user_ids=team_slack_ids)
        if meeting_docs:
    ...
    def _generate_slack_knowledge(self, since: datetime, until: datetime,
        ...
        mode = "チーム" if team_user_ids else "個人"
        logger.info(f"参加チャンネル数: {len(channels)}（{mode}モード）")
    ...
    def _save_slack_channel(self, channel_id: str, channel_name: str,
                             ...
                             week_num: str, week_label: str,
                             team_user_ids: list[str] = None) -> None:
        if team_user_ids:
            messages = self.slack.get_team_messages_in_channel(
                channel_id, channel_name, since, until, team_user_ids)
        else:
            messages = self.slack.get_my_messages_in_channel(channel_id, channel_name, since, until)
        ...
        if self.gemini and self.gemini.enabled:
            ...
            for msg in standalone:
                t = msg["datetime"].strftime("%m/%d %H:%M")
                user_name = msg.get("user_name") or "自分"
                mark = "★" if msg.get("is_mine", True) else ""
                all_msgs_text.append(f"[{t}] {mark}{user_name}: {(msg.get('text') or '')[:200]}")
            if all_msgs_text:
            ...
            "standalone_messages": [
                {
                    ...
                    "user_name": m.get("user_name") or "自分",
                    "is_mine": m.get("is_mine", True),
